...
void tx()
{
  ...
  for (i = 0; i < symbolCount; i++)
  {
    ...
    // t1 = millis();
    si5351.set_freq(frequency1 + (tones1[i] * toneSpacing), SI5351_CLK1);  // set_freq takes around 5ms!
    si5351.set_freq(frequency2 + (tones2[i] * toneSpacing), SI5351_CLK2);
    // t2 = millis();
    // Serial.println(t2 - t1);
    // delay(toneDelay - (t2 - t1));
    while ((micros() - starttime) < TONE_DELAY_ACCURATE) {}; // Not perfect but OK'ish for now!
  }
